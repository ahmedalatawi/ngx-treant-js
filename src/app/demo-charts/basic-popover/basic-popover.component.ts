...
export class BasicPopoverComponent implements AfterViewInit, OnInit {
	...
	constructor(private svc: DemoAppService, private modalService: BsModalService, private formBuilder: FormBuilder) {
	...
	onHover(event): void {
		...
		event
			...
			.on('click', '#delete', (e) => {
				let confirmDelete = confirm('Are you sure you want to remove employee: ' + event.node.text.name);
				...
				if (confirmDelete) {
					...
					setTimeout(() => {
								...
								: {
										...
										children:
											...
											node.children.reduce(
												(children, child) => children.concat(removeNode(child, id) || []),
												[]
											)
									};
						};
						...
						this.basicPopoverData.nodeStructure = removeNode(this.basicPopoverData.nodeStructure, node.id);
	...
	onLoadNodes(obj): void {
		...
		setTimeout(() => {
			this.flatNodes = this.svc.flattenItems([ this.basicPopoverData.nodeStructure ], 'children');
			this.flatNodes.forEach((n, i) => {
				this.flatNodes[i].id = i;
				const node = this.nodes.find((n) => n.id == i);
				this.flatNodes[i].parentId = node.parentId;
		...
		for (var id in mappedArr) {
			if (mappedArr.hasOwnProperty(id)) {
				...
				if (mappedElem.parentId) {
					mappedArr[mappedElem['parentId']] && mappedArr[mappedElem['parentId']]['children'].push(mappedElem);
				} else {
